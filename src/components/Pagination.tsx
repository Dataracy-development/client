...
export default function Pagination({ page, viewPerPage, total, onChange }: PaginationProps) {
    ...
    return (
        <div className="flex items-center gap-1.5">
            <button onClick={() => page > 1 && onChange(page - 1)} disabled={page === 1} className="w-10 h-9 flex items-center justify-center cursor-pointer">
            ...
            <button onClick={() => page < totalPage && onChange(page + 1)} disabled={page === totalPage} className="w-10 h-9 flex items-center justify-center cursor-pointer">
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
