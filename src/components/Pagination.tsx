...
export default function Pagination({ page, viewPerPage, total, onChange }: PaginationProps) {
    ...
    const renderPageNumbers = () => {
        ...
        // 항상 5개의 페이지 번호만 표시
        if (totalPage <= 5) {
            // 전체 페이지가 5개 이하일 경우 모든 페이지 번호 표시
    ...
    return (
        <div className="flex items-center gap-[7px]">
            <button onClick={() => page > 1 && onChange(page - 1)} disabled={page === 1} className="w-[30px] h-[30px] flex items-center justify-center cursor-pointer">
            ...
            <div className="flex gap-1.5">
                {renderPageNumbers().map((number, index) => (
                    <button
                        ...
                        className={`w-[30px] h-[30px] cursor-pointer text-sm font-normal ${page === number ? "text-black" : "text-gray-400"}`}
                    >
                        {number}
                    </button>
                ))}
            ...
            <button onClick={() => page < totalPage && onChange(page + 1)} disabled={page === totalPage} className="w-[30px] h-[30px] flex items-center justify-center cursor-pointer">
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
