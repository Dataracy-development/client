...
import { getUtilizeProjectsApi, UtilizeProject } from "../../_apis/apis";
        ...
        <div className="mb-12">
            <div className="bg-white border border-gray-200 rounded-2xl p-6">
                ...
                <div className="space-y-4">
                    {projects.data.content.length > 0 ? projects.data.content.map((project) => <UtilizeProjectRow key={project.id} project={project} />) : <NoUtilizeProjectMessage />}
                    {projects.data.totalElements > projects.data.content.length && <MoreBtn totalElements={projects.data.totalElements} contentLength={projects.data.content.length} />}
                </div>
            </div>
        </div>
    );
...
const UtilizeProjectRow = ({ project }: { project: UtilizeProject }) => {
...
const MoreBtn = ({ totalElements, contentLength }: { totalElements: number; contentLength: number }) => {
    return (
        <div className="text-center pt-4">
            <button className="inline-flex items-center gap-2 text-blue-600 hover:text-blue-700 text-sm font-medium px-4 py-2 rounded-lg hover:bg-blue-50 transition-colors">
                <span>+{totalElements - contentLength}개 프로젝트 더 보기</span>
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
