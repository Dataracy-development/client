    ...
    return (
        <div className="w-full">
            <div className="w-full flex justify-between items-center mb-2.5">
                <SearchInput />
                <div className="text-sm leading-[22px] font-normal text-n900">총 1,234개</div>
            </div>
...
const SearchInput = () => {
    return (
        <div className="w-[517px] h-9 px-3 bg-n200 rounded-md flex items-center gap-1.5">
            ...
            <input type="text" className="flex-1 focus:outline-none h-full text-sm leading-9 font-normal bg-n200" placeholder="Search" />
        </div>
    );
};
    ...
    return (
        <div className="w-full flex flex-wrap gap-2.5 items-center">
            {sortItems.map((item) => (
                <div
                    key={item.value}
                    className={`w-fit h-7 px-2 bg-n200 text-sm leading-7 font-normal rounded-[14px] cursor-pointer ${item.isActive ? "bg-n600 text-white" : "text-n700 bg-n200"}`}
                    onClick={() => {
                        onClick(item.value);
                    }}
                >
